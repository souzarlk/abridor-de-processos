...
function fileToGenerativePart(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onloadend=()=>{try{const result=String(reader.result||'');const comma=result.indexOf(',');if(comma<0)throw new Error(`Não foi possível preparar o PDF ${file.name}.`);resolve({inlineData:{data:result.slice(comma+1),mimeType:'application/pdf'}});}catch(e){reject(e)}};reader.onerror=()=>reject(reader.error||new Error('Não foi possível ler o PDF.'));reader.readAsDataURL(file);});}
...
async function analyzeWithGemini(formData){
  ...
  let fields;try{fields=JSON.parse(cleanJsonText(text));}catch(e){console.error('Resposta bruta da IA:',text);throw new Error('A IA retornou um formato inválido. Tente novamente.');}
...
window.fetch=async(input,init={})=>{const url=typeof input==='string'?input:(input?.url||'');const isAiRequest=url.includes('/api/extract-process')||url.includes('cloudfunctions.net/extractProcess');if(!isAiRequest)return originalFetch(input,init);try{return await analyzeWithGemini(init.body instanceof FormData?init.body:new FormData());}catch(error){console.error('Falha na análise com Firebase AI Logic:',error);return new Response(JSON.stringify({error:error?.message||'Não foi possível concluir a análise com a IA.'}),{status:500,headers:{'Content-Type':'application/json'}});}};
